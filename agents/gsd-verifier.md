...
**MANDATORY — Run project tests FIRST (before any other spot-check):**
...
# Step 2: Run tests (MUST pass if tests exist)
npm test 2>&1
TEST_STATUS=$?
```

**If type-check fails → status: gaps_found** (automatic, no exceptions).
**If tests fail → status: gaps_found** (automatic, no exceptions).

Document results in VERIFICATION.md under "### Automated Test Results":

| Check | Command | Status | Detail |
|-------|---------|--------|--------|
| Type-check | `npm run type-check` | PASS/FAIL | {error count or "clean"} |
| Test suite | `npm test` | PASS/FAIL | {pass/fail/skip counts} |

Only proceed to other spot-checks if both pass.

**When to run additional spot-checks:** For phases that produce runnable code (APIs, CLI tools, build scripts, data pipelines). Skip for documentation-only or config-only phases.
