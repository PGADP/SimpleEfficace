...
## Step 1.5: Search Internal History FIRST (mandatory)

**Before ANY external research, mine the project's own history.** The answer may already exist in `.planning/`.
...
2. **Read matching files** — Extract:
   - What was already tried (approaches, libraries, patterns)
   - What worked and what didn't (from VERIFICATION.md)
   - Decisions already made (from CONTEXT.md, SUMMARY.md)
   - Lessons learned (from deviations sections)

3. **Check STRATEGY-ARCHIVE.md** for relevant past decisions:
   ```bash
   cat .planning/STRATEGY-ARCHIVE.md 2>/dev/null
   ```

4. **Document in RESEARCH.md** as `## Internal History`:
   ```markdown
   ## Internal History (from .planning/)

   ### Related Past Phases
   | Phase | What was done | Outcome | Relevant findings |
   |-------|--------------|---------|-------------------|
   | 31.12 | Refactored analyst route | Split into 6 services | Pattern: extraction-pipeline orchestrator |

   ### Decisions Already Made
   - [Decision from phase X: ...]

   ### What Was Tried and Failed
   - [Approach from phase Y: didn't improve because ...]
   ```

**If internal history covers 80%+ of what's needed** → skip external research, mark confidence as HIGH (verified from own project), and proceed directly to writing RESEARCH.md.

**If internal history covers some but not all** → use it as starting point, only research the gaps externally.

**If no internal history found** → proceed to Step 2 normally.
