...
Ce step est optionnel et gouverne par la config `workflow.ui_front_pass` (defaut: `true`). Il invoque le skill projet `/front` (si present dans `.claude/commands/front.md`) pour enrichir la UI-SPEC generique produite par le researcher avec le design system specifique du projet.

**Pourquoi cette etape existe :** `gsd-ui-researcher` produit une spec correcte mais generique (regles universelles : max 4 tailles, 60/30/10, CTA specifiques). Elle ne connait pas le DS specifique du projet (tokens Tailwind reels, composants existants, exceptions DS historiques). Le skill `/front` est l'expert DS du projet — il comble ce fosse avant le checker, ce qui evite les revisions mecaniques (checker qui bloque sur des regles generales alors que la spec pourrait etre ancree dans le DS projet).
...
FRONT_SKILL_PATH=".claude/commands/front.md"
```
...
- Le fichier `.claude/commands/front.md` n'existe pas (projet sans skill `/front`)
...
**Spawn `/front` via Skill tool (si toutes conditions remplies) :**
...
◆ Running /front skill on UI-SPEC to anchor the spec in the project's design system...
```
...
**Handle front skill return:**
- Si le skill retourne un resume "Resume enrichissement UI-SPEC" avec fichier modifie → display resume, continue step 7
...
**Note :** le step 6.5 ne bloque jamais le workflow. Meme en cas d'echec, on passe au checker. C'est un step **best-effort** d'enrichissement qui ameliore la qualite si le projet a un skill `/front`, mais n'est pas critique au fonctionnement du workflow.
...
- [ ] Front skill pass attempted (if `workflow.ui_front_pass=true` and `.claude/commands/front.md` exists) — best-effort, non-blocking
