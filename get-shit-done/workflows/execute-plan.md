...
After each `type="auto"` task that modifies files under `src/`:

1. Check if `package.json` has a `"test"` script:
   ```bash
   grep '"test":' package.json | head -1
   ```

2. If test script exists AND task modified `src/` files:
   ```bash
   npm run test 2>&1 | tail -20
   ```

3. If tests fail:
   - Read failure output to identify broken tests
   - Fix the regression (this task broke something)
   - Re-run `npm run test` to confirm green
   - Only then proceed to commit

4. If no test script or no `src/` files modified: skip gate.

**Rationale:** Prevents shipping broken tests. Each task that touches source code must leave the test suite green.
</test_gate>
