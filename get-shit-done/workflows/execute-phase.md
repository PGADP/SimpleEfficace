...
<step name="update_reference_docs">
**Update reference documentation to prevent architecture drift.**

After phase completion, check if structural changes were made:

1. **Check for new services:** Compare `docs/reference/SERVICES_INDEX.md` (if exists) against actual service files:
```bash
# Find services created in this phase's commits
git log --name-only --diff-filter=A --grep="${PHASE_NUMBER}" --pretty="" | grep -E "\.service\.ts$" | sort -u
```

2. **If new services found:** Update `docs/reference/SERVICES_INDEX.md`:
   - Add each new service with its role and file path
   - Keep the file sorted by domain/directory

3. **If architecture changed** (new data flows, new pipeline steps, new API routes):
   - Update `docs/reference/ARCHITECTURE.md` with the changes
   - Keep descriptions concise — what changed, not the full history

4. **Commit:**
```bash
DOCS_CHANGED=$(git status --short docs/reference/ | wc -l)
if [ "$DOCS_CHANGED" -gt 0 ]; then
  node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE_NUMBER}): update reference documentation" --files docs/reference/SERVICES_INDEX.md docs/reference/ARCHITECTURE.md
fi
```
