---
name: pilot:briefing
description: Sous-skill du Pilot — briefing de début de session. Lit STATE.md consolidé + invoque /planning, produit un briefing 15 lignes. Chargé à la demande par /pilot. Non destiné à l'invocation directe.
...
Procédure lourde du Mode 1, chargée à la demande pour garder /pilot mince.

1. **Demande ce qui a ete fait** depuis la derniere session (taches manuelles, avancees hors code).

2. **OBLIGATOIRE : invoque `/planning`** via Skill, argument "Point planning debut de session — verifier etat reel des phases, recaler vue semaine si necessaire, alertes". /planning lit STRATEGY/ROADMAP/STATE, verifie la coherence, retourne un PLANNING STATUS structure. **Ne lis pas les fichiers toi-meme** — /planning garantit la verification systematique.

3. **Briefing 15 lignes max** depuis le retour de /planning :
...
**Note marketing** : ne lis PAS `.planning/marketing/`. Si l'utilisateur demande le marketing → rediriger vers `/pilot-marketing`.
...
Retour au sparring /pilot pour la suite.
