...
- **FLAG** (recommandation non-bloquante) : Défaut signalé mais ne bloque pas l'approbation.
- **PASS** : Pas de défaut trouvé.
...
<documented_ds_exceptions>

## Documented Design System Exceptions — DOWNGRADE rule

**Context:** Some projects have a mature design system with historical exceptions that are documented, intentional, and well-known in the codebase. Mechanically blocking on these exceptions creates friction without value — the checker should recognize documented exceptions and **downgrade the verdict from BLOCK to FLAG**.

**How this works:**

Before emitting a BLOCK on any dimension, scan the UI-SPEC for a section titled (case-insensitive variants accepted):
- `## Documented DS Exceptions`
- `## Exceptions DS documentees`
- `### Exception documentee` (within a dimension section)
- `### Exceptions documented` (within a dimension section)

If the issue detected is **explicitly covered** by a documented exception with a clear reason, DOWNGRADE the severity from BLOCK to FLAG.

**Examples of valid documented exceptions (accepted, downgraded):**

- `Badge uses text-xs (12px)` — accepted if documented as historical convention on a shared component used across the app. Reason must be explicit (e.g., "used in Badge.tsx shipped in all screens, resizing breaks alignment").
- `44px touch target` — accepted if justified by WCAG 2.5.5 or Apple HIG. Reason must cite the standard.
- `font-display uppercase 12px` — accepted if justified by ASCII font readability constraint.
- `5th font size in legacy component` — accepted if documented as "replaced progressively, phase N".

**Examples of INVALID exceptions (still BLOCK, do not downgrade):**

- "Designer wanted it" — not a reason
- "We always do it this way" — not documented
- Vague justification like "it looks better" — not a standard or constraint
- Newly introduced exception in this phase (not historical) — should be debated, not grandfathered

**Boundary of the rule:**

The DOWNGRADE rule applies to **Typography**, **Spacing**, and **Color** dimensions. It does NOT apply to **Copywriting** (generic CTAs are never excusable) nor to **Registry Safety** (third-party blocks without gate are non-negotiable).

**How to report a downgrade in verdict:**

When downgrading BLOCK → FLAG because of a documented exception, explicitly state:
```
Dimension N — {Name}: FLAG (documented exception)
  Issue: {short description}
  Exception source: {section/line where it's documented in UI-SPEC}
  Reason cited: {the explicit reason}
  Note: would be BLOCK without the documented exception
```

This preserves auditability — reviewers can see what was downgraded and why.
...
**Downgrade BLOCK → FLAG when:** a 5th font size is explicitly documented as a DS Exception with a clear reason (see `<documented_ds_exceptions>` block above). Example valid reason: "Badge uses text-xs (12px) — shipped component, resizing breaks alignment across all screens".
...
**Downgrade BLOCK → FLAG when:** a non-multiple-of-4 value is justified by an accessibility or interaction standard. Examples: `44px` touch targets (WCAG 2.5.5 / Apple HIG), specific `min-h` values for textareas tied to word capacity when documented. The justification must be explicit and cite the standard.
