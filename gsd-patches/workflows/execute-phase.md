...
- Prior VERIFICATION.md files from earlier phases (regression check). Look in BOTH .planning/phases/ AND .planning/_archive/phases/ : /se-archive moves shipped phases there, so searching only phases/ silently skips every phase completed before the last archival pass.
` : ''}
</required_reading>
...
if [ "$INDEX_CHANGED" -gt 0 ]; then
  gsd_run commit "docs(phase-${PHASE_NUMBER}): update INDEX.md at phase completion" --files .planning/INDEX.md
else
