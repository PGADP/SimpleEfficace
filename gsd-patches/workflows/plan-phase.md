...
**If `has_context` is true (CONTEXT.md exists from discuss-phase):** this alone is NOT a reason to skip. CONTEXT.md records the product decisions taken with the human (what we want). RESEARCH.md verifies external facts (how the dependency actually behaves, which version, which known pitfalls). A discussion cannot check an API contract. Fall through to the question below.
