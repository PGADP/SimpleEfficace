...
**Reference documentation (MANDATORY):** Before creating any plan, read these files if they exist:
- `docs/reference/ARCHITECTURE.md` — understand current architecture before proposing changes
- `docs/reference/SERVICES_INDEX.md` — know which services already exist to avoid duplication
If a phase requires creating functionality that overlaps with an existing service, plan to extend the existing service rather than creating a new one. Reference specific existing services by name in task actions.
...
**Nyquist Rule** (read `workflow.nyquist_validation` in `.planning/config.json` first, absent = enabled):

- **Enabled:** every `<verify>` must include an `<automated>` command. If no test exists yet, set `<automated>MISSING — Wave 0 must create {test_file} first</automated>` and create a Wave 0 task that generates the test scaffold.
- **Explicitly `false`:** prefer an `<automated>` command whenever one already exists, but never invent a Wave 0 scaffolding task for it. A manual verification is a valid `<verify>` in that mode.
...
**Every task that creates a new service file (.service.ts) MUST include writing a test file (.test.ts).**

The test file must cover at minimum:
- Happy path (normal input → expected output)
- Null/missing data (0 rows from DB, null parameters, empty strings)
- Error handling (invalid input, DB errors, network failures)

Include the test as part of the task action, not as a separate task. The executor will run `npm run type-check` and `npm test` before committing — both must pass. This is non-negotiable: untested services are the #1 source of production crashes.
...
**REFACTOR (if needed):** Clean up → run tests (MUST pass) → commit only if changes: `refactor({phase}-{plan}): clean up [feature]`
