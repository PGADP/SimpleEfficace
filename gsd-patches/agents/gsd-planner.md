...
- **Enabled:** every `<verify>` includes `<automated>`. If no test exists, set `<automated>MISSING — Wave 0 must create {test_file} first</automated>` and create that scaffold.
...
| `ui` | Yes | `true` if `files_modified` contains front code. Drives the executor design protocol. |
...
Three types: **checkpoint:human-verify (90%)**, **checkpoint:decision (9%)**, **checkpoint:human-action (1% - rare)**. Full "use for" criteria and XML templates for each: @$HOME/.claude/gsd-core/references/checkpoints.md
...
See @$HOME/.claude/gsd-core/references/planner-guidance.md for return formats; gap-closure returns are artifact-based (#3440).
