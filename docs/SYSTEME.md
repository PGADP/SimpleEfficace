...
| **branch-gate** | `PreToolUse` Bash (`git commit`/`merge`/`push`) | HEAD sur `main`, `master` ou `production` | refuse, et donne la commande de création de branche depuis `origin/main` ; laisse passer le `merge --ff-only` de release et les opérations sans nom de branche stable (rebase, HEAD détachée) (`workflow.branch_gate`) | **oui** |
| **branch-sweep** | `SessionStart` | dépôt avec un remote `origin` | annonce les branches réellement fusionnées (PR fusionnée, merge commit, patch-id, squash reconstruit) et élague les worktrees morts ; ne supprime qu'avec `workflow.branch_sweep` | non (annonce) |

**La loi de branche vit dans `CONVENTIONS.md` §13**, pas ici : une phase égale une branche, rien sur `main` ni `production`, une branche égale une PR, un chat égale un dossier, la branche meurt à la fusion.
...
**Note d'implémentation :** les hooks sont des scripts Node `.cjs` (`hooks/`), thin adapters stdin/stdout ; les 7 détecteurs advisory vivent dans `guard-lib.cjs` (testable, dispatché par `se-guard.cjs`), les 6 gates bloquantes sont des scripts dédiés. Peu de scripts : un dispatcher unique pour l'advisory, pas un script par détecteur.
