...
→ **Lance `/init "<ton idée>"`** pour le cadrer (pilot → brainstorm → PRD → research → roadmap).
→ Ou `/pilot` pour discuter d'abord avec le cofondateur.
...
- **Skills** (`.claude/commands/`) : `/pilot`, `/init`, `/ui`, `/ux`, `/research`, `/humanizer`, `/dev`, `/review`, `/test`, `/gate-simplify`, `/gate-janitor`, etc.
- **Garde-fous** (`.claude/settings.json` → `hooks/`) — actifs au démarrage de session :
  - `humanizer-guard` : rappelle `/humanizer` sur le contenu user-facing
...
- **Cycle GSD enrichi** (`get-shit-done/`) : gates simplify/janitor/visual-checkpoint actives (cf. `.planning/config.json`).
...
*(À confirmer / adapter via `/init` selon le projet.)*
